...
describe("crystal camera math", () => {
  test("maps X toward the viewer, Y right, Z up and preserves handedness through axis views and roll", () => {
    expect(CRYSTAL_SCREEN_AXES.map(({ label, direction }) => [label, direction])).toEqual([
      ["X", "outward"], ["Y", "right"], ["Z", "upward"],
    ]);
    const [x, y, z] = CRYSTAL_SCREEN_AXES.map(axis => new Vector3(...axis.viewVector)) as [Vector3, Vector3, Vector3];
    expect(x.clone().cross(y).dot(z)).toBeCloseTo(1);
    for (const axis of CRYSTAL_SCREEN_AXES) {
      const state = stateWithDirectAxis(CUBIC_CELL, {
        ...createDefaultCrystalCameraState(CUBIC_CELL), primary: axis.direction,
        secondary: defaultSecondaryDirectionForPrimary(axis.direction),
      }, "a");
      for (const roll of [0, 90, -120]) {
        const vectors = computeCrystalCameraVectors(CUBIC_CELL, applyCrystalCameraRoll(CUBIC_CELL, state, roll));
        const frame = { outward: vectors.outward, right: vectors.right, upward: vectors.up };
        expectVectorClose(frame[axis.direction], [1, 0, 0]);
        expect(frame.outward.clone().cross(frame.right).dot(frame.upward)).toBeCloseTo(1);
      }
    }
  });
