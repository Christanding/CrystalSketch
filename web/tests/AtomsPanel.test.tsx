...
describe("AtomsPanel", () => {
  test("keeps global radius model and scale above the object controls", async () => {
    ...
    expect(modelSelect.textContent).toContain("Atomic");
    expect(scaleSlider.value).toBe("55");
    expect(scaleInput.value).toBe("55");
  ...
  test("lists deleted atom images separately and restores them without clearing hidden appearance", async () => {
    const user = userEvent.setup();
    const source = sceneWithAtoms(2);
    source.atoms = source.atoms.map(atom => ({ ...atom, sourceAtomNumber: atom.siteIndex === 0 ? 7 : 2 }));
    const style = createDefaultStyle();
    style.objectStyles = setAtomOverrideProperty(style.objectStyles, "Na-1", "visible", false);
    render(<AtomsPanelHarness initialSelectedAtomId={null} initialStyle={style} scene={source}
      initialDeleted={{ atoms: ["Na-1", "Na-0-image-1-0-0"], bonds: [] }} />);
    expect(screen.getByRole("button", { name: "Deleted atoms 2" }).getAttribute("aria-expanded")).toBe("false");
    await user.click(screen.getByRole("button", { name: "Deleted atoms 2" }));
    expect(screen.getByText("Na:7 [1, 0, 0]").isConnected).toBe(true);
    await user.click(screen.getByRole("button", { name: "Restore atom Na:2" }));
    expect(screen.getByRole("button", { name: "Deleted atoms 1" }).isConnected).toBe(true);
    expect(screen.getByRole("button", { name: "Hidden atoms 1" }).isConnected).toBe(true);
    expect(screen.getByRole("button", { name: "Restore atom Na:7 [1, 0, 0]" }).isConnected).toBe(true);
    await user.click(screen.getByRole("button", { name: "Restore atom Na:7 [1, 0, 0]" }));
    expect(screen.queryByText("Deleted atoms")).toBeNull();
    expect(screen.getByRole("button", { name: "Hidden atoms 1" }).isConnected).toBe(true);
    await user.click(screen.getByRole("button", { name: "Undo restore" }));
    expect(screen.getByRole("button", { name: "Deleted atoms 1" }).isConnected).toBe(true);
    expect(screen.getByRole("button", { name: "Restore atom Na:7 [1, 0, 0]" }).isConnected).toBe(true);
    expect(screen.queryByRole("button", { name: "Restore atom Na:7" })).toBeNull();
  });
});
...
function AtomsPanelHarness({
  ...
  const editing = useSceneEdits(scene, 0, initialDeleted ? { deleted: initialDeleted, history: [] } : undefined);
  ...
  return (
    <ColorPickerRegistryProvider>
      <AtomsPanel
        ...
        scene={editing.scene!}
        sourceScene={initialDeleted ? scene : undefined}
