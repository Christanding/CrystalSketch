...
describe("settings", () => {
  test("keeps the large-scene mesh quality threshold", () => {
    ...
    expect(defaultPreviewMeshQualityForScene(null)).toBe("high");
    expect(defaultPreviewMeshQualityForScene(sceneWithAtomCount(belowThreshold))).toBe("high");
  ...
  test("keeps original cartoon sizes with the CrystalSketch fresh anime palette and disabled fading", () => {
    expect(createDefaultStyle()).toEqual({
      ...
      customColormap: { baseColorScheme: "vesta-soft", elements: expect.objectContaining({Cu:"#f1abbc",I:"#97cff2",Zn:"#b39ae6",S:"#f3da78"}) },
      distinguishSimilarColors: false,
      fogAffectsUnitCell: false,
      fogAmount: 40,
      fogEnabled: false,
      fogStart: 40,
      materialPreset: "cartoon",
      objectStyles: {
        atomOverrides: {},
        bondFamilyOverrides: {},
        bondOverrides: {},
  ...
  test("changes only DPI metadata settings, preserves pixel budgets, and accepts legacy defaults", () => {
    const original = createDefaultExportSettings();
    expect(EXPORT_DPI_OPTIONS).toEqual([300, 400, 600, 700, 800, 1000, 1200]);
    for (const dpi of EXPORT_DPI_OPTIONS) {
      const next = setExportDpi(original, dpi);
      expect(next).toEqual({ ...original, dpi });
      expect(validateExportSettings(next)).toEqual({ valid: true, code: null });
      expect(validateExportSettings({ ...next, width: 2001 })).toEqual({ valid: false, code: "size-too-large" });
    }
    expect(exportDpi({})).toBe(600);
    expect(validateExportSettings({ ...original, dpi: undefined }).valid).toBe(true);
    for (const dpi of [0, 72, 150, 500, NaN, Infinity]) {
      expect(setExportDpi(original, dpi)).toBe(original);
      const invalid = { ...original, dpi } as ExportSettingsState;
      expect(validateExportSettings(invalid)).toEqual({ valid: false, code: "dpi-invalid" });
      expect(() => exportDpi(invalid)).toThrow();
    }
  });
  ...
  test("parses and validates bounded export quality settings", () => {
    ...
    expect(setExportSupersampling(defaultSettings, 9).supersampling).toBe(2);
    ...
    expect(setExportBackground(defaultSettings, "black").background).toBe("white");
    expect(validateExportSettings({ ...defaultSettings, background: "black" }).valid).toBe(false);
    ...
    expect(validateExportSettings({ ...defaultSettings, width: 4000, height: 4000, supersampling: 1 }).valid).toBe(true);
    expect(validateExportSettings({ ...defaultSettings, width: 5000, height: 5000, supersampling: 1 })).toEqual({ valid: false, code: "size-too-large" });
    expect(validateExportSettings({ ...defaultSettings, width: 3000, height: 3000, supersampling: 2 })).toEqual({ valid: false, code: "size-too-large" });
  ...
  test("uses a stable right safe area and a small inspector scene offset", () => {
    ...
    expect(safeArea.left).toBe(344);
    expect(safeArea.bottom).toBe(116);
    expect(safeArea.top).toBe(88);
