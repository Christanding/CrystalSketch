...
describe("hide selected object shortcut", () => {
  ...
  test("stops responding while its view is inactive and resumes without losing selection", () => {
    const onHideAtom = mock();
    const { rerender } = render(<ShortcutHarness active={false} onHideAtom={onHideAtom} selectedAtomId="Na-0" />);
    fireEvent.keyDown(window, { key: "h" });
    expect(onHideAtom).not.toHaveBeenCalled();
    rerender(<ShortcutHarness active onHideAtom={onHideAtom} selectedAtomId="Na-0" />);
    fireEvent.keyDown(window, { key: "h" });
    expect(onHideAtom).toHaveBeenCalledTimes(1);
    rerender(<ShortcutHarness active={false} onHideAtom={onHideAtom} selectedAtomId="Na-0" />);
    fireEvent.keyDown(window, { key: "h" });
    expect(onHideAtom).toHaveBeenCalledTimes(1);
  });
});
