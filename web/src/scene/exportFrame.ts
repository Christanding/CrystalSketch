...
import { layoutMeasurementLabels, MEASUREMENT_LABEL_CENTER, measurementLabelSize } from "../model/measurementLabelLayout";
import {
...
import { computeSceneStructureLayout } from "./sceneLayout";
import {
  MEASUREMENT_LINE_WIDTH_RATIO,
  displayedMeasurements,
  measurementAngleArcPositions,
  measurementLayoutObstacles,
...
export function computeStructureProjectedBounds({
  ...
  if (measurements.length > 0) {
    const { span } = computeSceneStructureLayout(scene);
    const lineRadius = span * MEASUREMENT_LINE_WIDTH_RATIO / 2;
    const labelPositions = scene.measurementStyle?.showLabels === false ? null : layoutMeasurementLabels({
      measurements,
      cameraQuaternion: cameraPose.quaternion,
      span,
    ...
    for (const measurement of measurements) {
      for (const point of measurement.points) {
        bounds.includePoint(projector.projectPoint(point), lineRadius);
      }
      const arc = measurementAngleArcPositions(measurement);
      for (let index = 0; index < arc.length; index += 3) {
        bounds.includePoint(projector.projectPoint([arc[index]!, arc[index + 1]!, arc[index + 2]!]), lineRadius);
      }
      if (scene.measurementStyle?.showLabels === false) continue;
      // Sprites face the camera: their extents belong in the camera plane, after projection.
      const anchor = projector.projectPoint(labelPositions?.get(measurement.definition.id) ?? measurement.labelPosition);
      const { width, height } = measurementLabelSize(measurement.label, span, scene.measurementStyle?.fontScale);
      bounds.includePoint({
        x: anchor.x - width * MEASUREMENT_LABEL_CENTER[0],
        y: anchor.y - height * MEASUREMENT_LABEL_CENTER[1],
      });
      bounds.includePoint({
        x: anchor.x + width * (1 - MEASUREMENT_LABEL_CENTER[0]),
        y: anchor.y + height * (1 - MEASUREMENT_LABEL_CENTER[1]),
      });
    }
  }
