...
// Native Three.js camera coordinates; do not substitute the user-facing XYZ labels here.
...
const DEFAULT_SECONDARY_DIRECTION: Record<
  CrystalCameraScreenDirection,
  CrystalCameraScreenDirection
> = Object.fromEntries(CRYSTAL_SCREEN_AXES.map((axis, index) => [
  axis.direction, CRYSTAL_SCREEN_AXES[(index + 1) % CRYSTAL_SCREEN_AXES.length]!.direction,
])) as Record<CrystalCameraScreenDirection, CrystalCameraScreenDirection>;
const SCREEN_DIRECTION_INDEX = Object.fromEntries(CRYSTAL_SCREEN_AXES.map((axis, index) =>
  [axis.direction, index])) as Record<CrystalCameraScreenDirection, number>;
