...
import { withPngDpi } from "../export/pngDpi";
...
export async function renderStructureRasterImage({
  ...
  try {
    ...
    root.unmount();
    (rootState as RootState | null)?.gl.dispose();
...
export async function renderCrystalAxesRasterImage({
  ...
  const axes = computeOrientationGizmoAxes(cellVectors, axisColors);
  const root = createRoot(canvas);
  let rootState: RootState | null = null;
  let resolveMounted: (() => void) | null = null;
  const mounted = new Promise<void>((resolve) => {
...
function RenderReady({ onReady }: { onReady: () => void }) {
  // Advance camera/light subscribers without two redundant full GPU renders.
  // Each export path explicitly renders once after those subscribers settle.
  useFrame(() => {}, 1);
  useLayoutEffect(() => {
    onReady();
  }, [onReady]);
...
function canvasToRasterBlob(
  ...
  }).then(blob => imageFormat === "png" ? withPngDpi(blob) : blob);
}
