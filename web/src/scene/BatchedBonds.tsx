...
export function BatchedBonds({
  ...
  onInspect?: (bondId: string | null, additive?: boolean) => void;
  ...
  useLayoutEffect(() => {
    ...
    pickRegistryRef.current.itemByBatchId.forEach((item, batchId) => {
      setBatchedInstanceRgba(mesh, batchId,
        new Color(selection.bonds.has(item.id) ? "#c2415a" : "#ffffff"), item.opacity);
    });
    invalidate();
  }, [batch, invalidate, selection.bonds]);
  const handleClick = useCallback(
    (event: ThreeEvent<MouseEvent>) => {
      ...
      if (action === "locked-feedback") {
        ...
        onInspect?.(item.id, event.metaKey || event.ctrlKey);
  ...
  const isTransparent = batch.items.some((item) => item.opacity < 1);
...
function populateBatchedBondMesh(
  ...
  const instanceColor = new Color(0xffffff);
  ...
  for (const item of batch.items) {
    ...
    setBatchedInstanceRgba(mesh, batchId, instanceColor, item.opacity);
...
export function bondBatchPopulationKey({
  colorMode,
  items,
  radialSegments,
  radius,
