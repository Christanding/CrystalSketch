...
import cartoonPresetData from "../data/material-presets/presets/cartoon.json";
import softCeramicPresetData from "../data/material-presets/presets/soft-ceramic.json";
import satinMattePresetData from "../data/material-presets/presets/satin-matte.json";
import celShadedPresetData from "../data/material-presets/presets/cel-shaded.json";
import coloredMetalPresetData from "../data/material-presets/presets/colored-metal.json";
import softJadePresetData from "../data/material-presets/presets/soft-jade.json";
import softVelvetPresetData from "../data/material-presets/presets/soft-velvet.json";
...
const STATIC_MATERIAL_PRESET_MODULES: Record<string, unknown> = {
  ...
  "../data/material-presets/presets/cel-shaded.json": celShadedPresetData,
  "../data/material-presets/presets/colored-metal.json": coloredMetalPresetData,
  "../data/material-presets/presets/soft-jade.json": softJadePresetData,
  "../data/material-presets/presets/soft-velvet.json": softVelvetPresetData,
...
const MATERIAL_PROP_ALLOWLIST: Record<
  MaterialPresetMaterialType,
  ReadonlySet<string>
> = {
  MeshToonMaterial: new Set(["emissive", "emissiveIntensity"]),
  MeshPhongMaterial: new Set(["shininess", "specular", "flatShading"]),
  MeshBasicMaterial: new Set(),
  MeshLambertMaterial: new Set([
    "emissive",
    "emissiveIntensity",
    "flatShading",
  ]),
  MeshPhysicalMaterial: new Set([
    "attenuationColor",
    "attenuationDistance",
    "clearcoat",
    "clearcoatRoughness",
...
export function isIllustrationMaterialPreset(id: MaterialPresetId): boolean {
  return ["cartoon", "soft-ceramic", "satin-matte", "cel-shaded", "soft-jade", "soft-velvet"].includes(id) || isMetalMaterialPreset(id);
}

export function isMetalMaterialPreset(id: MaterialPresetId): boolean {
...
function parseMaterial(data: unknown, path: string): MaterialPresetMaterial {
  ...
  const type = expectMaterialType(material.type, `${path}.type`);

  return {
    props: expectMaterialProps(material.props, `${path}.props`, type),
    type,
  };
}
...
function expectMaterialProps(
  data: unknown,
  path: string,
  type: MaterialPresetMaterialType,
): MaterialPresetProps {
  const props = expectProps(data, path);
  const allowedProps = MATERIAL_PROP_ALLOWLIST[type];
