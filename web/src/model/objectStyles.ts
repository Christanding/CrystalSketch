...
export function setAtomColorOverrides(
  objectStyles: ObjectStyleState,
  atoms: readonly AtomSpec[],
  color: string | null,
): ObjectStyleState {
  const keys = new Set(atoms.flatMap(atom => color === null ? [atom.id, atomObjectStyleKey(atom)] : [atomObjectStyleKey(atom)]));
  const atomOverrides = { ...objectStyles.atomOverrides };
  let changed = false;
  for (const key of keys) {
    const current = atomOverrides[key];
    if (color === null) {
      if (current?.color === undefined) continue;
      const next = removeAtomOverrideProperty(current, "color");
      if (hasAtomOverride(next)) atomOverrides[key] = next;
      else delete atomOverrides[key];
    } else {
      if (current?.color === color) continue;
      atomOverrides[key] = { ...current, color };
    }
    changed = true;
  }
  return changed ? { ...objectStyles, atomOverrides } : objectStyles;
}
