...
export async function createLegendExportFile({
  ...
  return {
    blob: await canvasToRasterBlob(renderedLegend.canvas, rasterFormatForExportFormat(format), dpi),
    fileName,
    format,
  };
}
...
function legendFont(style: LegendExportStyle) {
  return `400 ${style.fontSize}px ${FIGURE_FONT_FAMILY}`;
}
