...
export function canvasToRasterBlob(
  ...
  return format === "jpg" ? canvasToJpgBlob(canvas, dpi) : canvasToPngBlob(canvas, dpi);
}

export function withRasterDpi(blob: Blob, format: RasterExportFileFormat, dpi: number = DEFAULT_EXPORT_DPI): Promise<Blob> {
  return format === "jpg" ? withJpegDpi(blob, dpi) : withPngDpi(blob, dpi);
}
...
export function canvasToPngBlob(canvas: HTMLCanvasElement, dpi: number = DEFAULT_EXPORT_DPI): Promise<Blob> {
  ...
  }).then(blob => withPngDpi(blob, dpi));
}

function canvasToJpgBlob(canvas: HTMLCanvasElement, dpi: number = DEFAULT_EXPORT_DPI): Promise<Blob> {
  const outputCanvas = document.createElement("canvas");
  ...
  }).then(blob => withJpegDpi(blob, dpi));
}
