...
export interface SceneSpec {
  measurements?: readonly SceneMeasurement[];
...
export type BondAlgorithm = "crystal-nn" | "minimum-distance" | "cut-off-dict" | "cartoon-distance";
...
export const STATIC_SCENE_PREVIEW_URL =
  import.meta.env.VITE_CRYSTALSKETCH_STATIC_SCENE ?? "";
export const STATIC_SCENE_PREVIEW_NAME =
  import.meta.env.VITE_CRYSTALSKETCH_STATIC_SCENE_NAME ?? "Example structure";
...
export async function loadStaticScenePreview(signal?: AbortSignal): Promise<SceneSpec | null> {
  ...
  try {
    response = await fetch(STATIC_SCENE_PREVIEW_URL, { signal });
  } catch (error) {
...
export async function uploadStructurePreview(
  ...
  if (isVaspFilename(file.name)) {
    return parseVaspFile(file, options);
  }
  if (hasStaticScenePreview()) {
  ...
  const headers: Record<string, string> = {
    ...
    "x-crystalsketch-filename": encodeURIComponent(file.name),
  };
  if (options.cutoffOverrides) {
    headers["x-crystalsketch-bond-cutoff-overrides"] = JSON.stringify(
