...
export function ElementLegend({
  ...
  return (
    <nav
      ...
      style={{ ...legendContainerStyle(safeArea, offsetX), ...style }}
    >
      <ul className={cn("flex flex-wrap items-center justify-center gap-x-4 gap-y-2", compact && "pointer-events-auto flex-nowrap justify-start gap-x-3 overflow-x-auto overscroll-x-contain")}>
        {entries.map((entry) => (
          <li key={entry.element} className={cn("flex min-w-0 items-center gap-2", compact && "shrink-0")}>
...
function legendContainerStyle(safeArea: PreviewSafeArea, offsetX: number): CSSProperties {
  return {
    ...
    maxWidth: `min(calc(100% - ${safeArea.left + safeArea.right + 32}px), 760px)`,
  };
}
