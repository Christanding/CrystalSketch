...
import { CrystalSketchLogo } from "./CrystalSketchLogo";

const CRYSTAL_SYSTEM_LABELS = {
  triclinic: "summary.crystalSystems.triclinic",
  monoclinic: "summary.crystalSystems.monoclinic",
  orthorhombic: "summary.crystalSystems.orthorhombic",
  tetragonal: "summary.crystalSystems.tetragonal",
  trigonal: "summary.crystalSystems.trigonal",
  hexagonal: "summary.crystalSystems.hexagonal",
  cubic: "summary.crystalSystems.cubic",
} as const;
...
export function StructureSummaryCard({
  ...
  onClearWorkspace,
  clearDisabled = false,
  canClearRecovery = false,
  symmetryPending = false,
  previewStatus,
  scene,
  selectedFileName,
}: {
  ...
  canClearRecovery?: boolean;
  symmetryPending?: boolean;
  ...
  const crystalSystem = summary.symmetry?.crystalSystem;
  const crystalSystemLabel = crystalSystem && Object.hasOwn(CRYSTAL_SYSTEM_LABELS, crystalSystem)
    ? t(CRYSTAL_SYSTEM_LABELS[crystalSystem as keyof typeof CRYSTAL_SYSTEM_LABELS]) : "-";
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-1.5">
          {onCollapseSidebar ? <TooltipProvider delayDuration={500}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="size-7 shrink-0 rounded-md bg-transparent p-0 shadow-none hover:bg-transparent"
                  aria-label={t("actions.hideLeftSidebar")} aria-expanded={true} aria-controls="left-controls-sidebar"
                  onClick={onCollapseSidebar}><CrystalSketchLogo className="size-7" /></Button>
              </TooltipTrigger>
              <TooltipContent side="right">{t("actions.hideLeftSidebar")}</TooltipContent>
            </Tooltip>
          </TooltipProvider> : <CrystalSketchLogo className="size-7 shrink-0" />}
        ...
        {onClearWorkspace ? <Button
          variant="ghost" size="sm" className="h-7 px-1 text-xs"
          aria-label={t("workspace.clearTitle")}
          disabled={clearDisabled || (!scene && previewStatus !== "loading" && !canClearRecovery)}
          onClick={onClearWorkspace}
        ><span>{t("workspace.clear")}</span></Button> : null}
        <Button
          size="sm"
      ...
      {symmetryPending ? <p role="status" className="mt-2 text-xs text-muted-foreground">{t("workspace.analyzing")}</p> : null}
