    ...
    >
      ...
      <TabsContent value="bonds" className="m-0 min-h-0">
        <div data-slot="bond-global-controls" className="mb-3 flex flex-col gap-1 text-[13px]">
          {scene.sourceFormat === "vasp" && onBondToleranceChange ? (
            <div className="grid h-7 grid-cols-[minmax(5.5rem,1fr)_9.6rem] items-center gap-2 rounded-md px-1.5">
              <span>{t("settings.bondTolerance")}</span>
              <NumberStepper aria-label={t("settings.bondTolerance")} min={.8} max={1.5} step={.05}
                value={scene.bondTolerance ?? 1.15} onValueChange={onBondToleranceChange} />
            </div>
          ) : null}
          <PercentSliderRow
          ...
          <div className="grid h-7 grid-cols-[minmax(5.5rem,1fr)_9.6rem] items-center gap-2 rounded-md px-1.5">
            ...
            >
              ...
              <SelectContent position="popper" className="!bg-background !text-foreground">
                <SelectGroup>
                  {(scene.sourceFormat === "vasp"
                    ? [{ value: "cartoon-distance", label: "距离 / 元素半径" }]
