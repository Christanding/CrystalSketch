import { Copy, EyeOff, SquareMousePointer, Trash2, X } from "lucide-react";
...
export function BondInspectorCard({
  ...
  return (
    ...
    >
      <div className="grid h-7 grid-cols-[1.5rem_minmax(0,1fr)_1.5rem_1.5rem_1.5rem_1.5rem] items-center gap-2">
        <CardAction
        ...
        <div className="flex min-w-0 items-center gap-1.5 whitespace-nowrap text-[0.78rem] font-semibold text-foreground">
          ...
          <span className="min-w-0 truncate">{atomSiteLabel(info.startAtom)}</span>
          <span aria-hidden="true" className="text-muted-foreground">—</span>
          <AtomToken color={endColor} />
          <span className="min-w-0 truncate">{atomSiteLabel(info.endAtom)}</span>
        </div>
        <CardAction
