...
export function StyleTabContent({
  ...
  const colorSchemeLabels = t("style.colorSchemes", { returnObjects: true });
            ...
            >
              <SelectGroup>
                {COLOR_SCHEME_OPTIONS.map((option) => {
                  const label = colorSchemeLabels[option.value as keyof typeof colorSchemeLabels] ?? option.label;
                  return (
                    <SelectItem
                      key={option.value}
                      value={option.value}
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-2">
      <span
        ...
        className="h-3 w-6 shrink-0 rounded-full border border-border"
        title={elements?.join(" / ")}
        style={colorSchemeTokenStyle(value, elements)}
      />
      <span className="min-w-0 truncate" title={label === sourceLabel ? label : `${label} · ${sourceLabel}`}>{label}</span>
    </span>
  );
}
