    ...
    <div className="flex flex-col gap-1.5">
          ...
          {hasPolyhedra ? (
            <div className="flex min-h-7 items-center justify-between gap-2 px-1.5 text-[13px]">
              ...
              <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-2">
                {polyhedronElements.map(element => (
                  <span key={element} className="inline-flex items-center gap-1.5">
                    <HexColorPicker
                      ariaLabel={t("colorPicker.polyhedronColor", { element })}
                      inputLabel={t("colorPicker.polyhedronColorValue", { element })}
                      pickerId={`display:polyhedron:${element}`}
                      side="right"
                      value={polyhedronColorForElement(style, element)}
                      onValueChange={color => onStyleChange(current => ({
                        ...current, polyhedronColors: { ...current.polyhedronColors, [element]: color },
                      }))}
                    />
                    <span>{element}</span>
                  </span>
                ...
                <Button
                  ...
                  aria-label={t("actions.resetPolyhedronColors")}
                  disabled={!Object.keys(style.polyhedronColors ?? {}).length}
                  onClick={() => onStyleChange(current => ({ ...current, polyhedronColors: {} }))}
      ...
      <section aria-labelledby="image-components-label">
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          ...
          />
          <label className={cn("flex h-7 cursor-pointer items-center justify-between gap-2 px-1.5", COMMON_PANEL_BODY_TEXT_CLASS)}>
            <span>{t("display.hideUnbondedBoundary")}</span>
            <Checkbox aria-label={t("display.hideUnbondedBoundary")}
              checked={visibility.hideUnbondedBoundary ?? false}
              className="size-3.5 rounded-[3px]"
              onCheckedChange={checked => setVisibility("hideUnbondedBoundary", checked === true)} />
          </label>
        </div>
      </section>
    </div>
...
function ComponentOpacityRow({
  ...
  const committedInput = useCommittedInput({
    ...
    parseValue: (valueText) => {
      const parsedValue = parseOpacityInput(valueText);
      return parsedValue === null ? null : clampOpacityValue(parsedValue, max);
    },
