...
import { CRYSTAL_SCREEN_AXES } from "../../../../model/crystalCameraState";
...
const ARROW_GEOMETRY_UP = new Vector3(0, 1, 0);
const SCREEN_AXIS_LABEL_POSITIONS = {
  upward: "left-[4.325rem] top-[0.65rem]",
  right: "left-[7.8rem] top-[5.15rem]",
  outward: "left-[1.6rem] top-[6.5rem]",
};
const SCREEN_AXIS_LABEL_KEYS = {
  upward: "orientation.screenAxisUp",
  right: "orientation.screenAxisRight",
  outward: "orientation.screenAxisOut",
} as const;
...
export function ScreenAxisChooser({
  ...
  return (
    ...
    >
      ...
      {CRYSTAL_SCREEN_AXES.map(axis => (
        <ScreenAxisOverlayLabel
          key={axis.direction}
          direction={axis.direction}
          hoveredAxis={hoveredAxis}
          selectedAxis={value}
          className={SCREEN_AXIS_LABEL_POSITIONS[axis.direction]}
        >{axis.label}</ScreenAxisOverlayLabel>
      ))}
      {CRYSTAL_SCREEN_AXES.map(({ direction }) => (
        <button
          key={direction}
          type="button"
          aria-label={t(SCREEN_AXIS_LABEL_KEYS[direction])}
          ...
          style={screenAxisHitboxStyle(direction)}
          onClick={() => onValueChange(direction)}
          onMouseEnter={() => setHoveredAxis(direction)}
          onMouseLeave={() => setHoveredAxis(null)}
        />
      ))}
    </div>
...
function ScreenAxisArrow({
  ...
  axis: (typeof CRYSTAL_SCREEN_AXES)[number];
  colors: PreviewThemeColors["screenAxis"];
  hovered: boolean;
  reducedMotion: boolean;
  selected: boolean;
  ...
  const axisDirection = useMemo(() => new Vector3(...axis.viewVector).normalize(), [axis.viewVector]);
  const axisRotation = useMemo(
    () => new Quaternion().setFromUnitVectors(ARROW_GEOMETRY_UP, axisDirection),
    [axisDirection],
  );
