...
const EXPORT_SEGMENTED_TRIGGER_CLASS =
  "!h-5 rounded-[4px] px-0.5 py-0 text-[0.68rem] font-medium transition-[background-color,color,box-shadow] duration-75 ease-out dark:data-[state=active]:bg-background/50 motion-reduced:transition-none md:text-[0.68rem]";

export function ExportTabContent({ structureContent, ...props }: FigureExportProps & { structureContent?: ReactNode }) {
  const { t } = useTranslation();
  if (!structureContent) return <FigureExportContent {...props} />;
  return (
    <Tabs defaultValue="figure">
      <TabsList variant="line" className="mb-2 w-full" aria-label={t("poscar.exportKind")}>
        <TabsTrigger value="figure">{t("poscar.figure")}</TabsTrigger>
        <TabsTrigger value="structure">{t("poscar.structure")}</TabsTrigger>
      </TabsList>
      <TabsContent value="figure"><FigureExportContent {...props} /></TabsContent>
      <TabsContent value="structure">{structureContent}</TabsContent>
    </Tabs>
  );
}
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <section aria-labelledby="export-output-label" className="flex flex-col gap-2.5">
        <div className="grid grid-cols-[minmax(5.5rem,1fr)_6.75rem_2.35rem] items-center gap-2 px-1.5">
          ...
          <div className="flex items-center justify-end gap-1.5">
            <span className={cn("text-muted-foreground", COMMON_PANEL_FIELD_LABEL_TEXT_CLASS)}>{t("exportPanel.dpi")}</span>
            <Select value={String(settings.dpi ?? 600)} onValueChange={value => onSettingsChange(setExportDpi(settings, Number(value)))}>
              <SelectTrigger size="sm" aria-label={t("exportPanel.dpi")} className="!h-6 w-17 !px-1.5 !py-0 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent><SelectGroup>
                {EXPORT_DPI_OPTIONS.map(dpi => <SelectItem key={dpi} value={String(dpi)}>{dpi}</SelectItem>)}
      ...
      <div className="mb-1.5 grid min-h-8 grid-cols-[auto_auto] items-end justify-between gap-2 px-1.5">
        ...
        <div className="flex items-center gap-1.5">
          {onPreview ? <Button size="sm" variant="outline" aria-label={t("figurePreview.title")}
            className="h-7 gap-1 rounded-full px-2 text-xs" disabled={!validation.valid || isExporting}
            onClick={onPreview}><Eye data-icon="inline-start" aria-hidden="true" />{t("poscar.previewAction")}</Button> : null}
          <Button
            size="sm"
...
function ExportSizeInput({
  ...
  const committedInput = useCommittedInput({
    value,
    formatValue: String,
    parseValue: parseExportDimensionInput,
    onCommit,
  });
