...
多面体保留原有“自动全部”模式。若只需某些原子的配位多面体，先选中中心原子（支持多选），再在“显示”中点击“按选中生成”；其余晶体保持原显示状态，周期配位原子的球体可继续隐藏。中心选择会按文档保存，预览与导出使用同一组多面体，也可撤销或切回自动模式。手动生成只使用当前成键关系，允许自动元素规则排除的中心；不足 4 个邻居、几何退化或外围镜像配位不完整时会说明原因，不会自动补键或修改结构。多面体开关变灰时，下方会提示尚在计算、配位不足或没有自动生成结果等状态。

测量标注的文字大小范围为 25%–250%，默认仍为 100%。在“导出 → 预览图片”中可直接拖动每条已保留的距离或角度文字，也可用方向键微调（Shift 为 10 像素）。合并预览和分开导出的结构预览均支持；位置按文档保存，并随导出尺寸按比例调整，PNG、JPG、PDF 使用相同位置。拖动只改变出图排版，不改变测量值、测量线或三维视图中的自动标注；无需重新光追采样。“重置”可恢复自动位置。
