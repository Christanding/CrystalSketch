...
STRUCTURE_PREWARM_MODULES = (
    "crystalsketch.structures.readers",
    "crystalsketch.structures.scene_builder",
)
...
def start_structure_preview_prewarm() -> threading.Thread:
    thread = threading.Thread(
        ...
        name="crystalsketch-structure-prewarm",
        daemon=True,
    )
    thread.start()
    return thread
...
def _prewarm_structure_preview_dependencies() -> None:
    try:
        ...
        logger.exception(
            ...
            "Structure loading may fail; restart with `Crystal --verbose` for request logs."
        )
