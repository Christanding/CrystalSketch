...
def _resolve_static_root(
    ...
    candidates.append(Path(str(resources.files("crystalsketch") / "web_static")))
        ...
        return HTMLResponse(
            ...
            <html lang="en">
              ...
              <body>
                <main>
                  <section>
                    <h1>CrystalSketch web app is unavailable</h1>
                    <p>
                      The local server is running, but its bundled browser files are
                      missing or incomplete.
                    </p>
                    <p>
                      Reinstall CrystalSketch using the same tool you installed it with,
                      then restart it. If the problem continues, run:
                    </p>
                    <code>Crystal --verbose</code>
                    <details>
