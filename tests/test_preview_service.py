...
import crystalsketch.server.preview_service as preview_service
import crystalsketch.structures.connectivity as connectivity_module
from crystalsketch.server.app import create_app
from crystalsketch.server.preview_service import RecentStructureCache
...
from crystalsketch.structures.scene_builder import build_scene_spec
        ...
        async def request_preview() -> None:
            responses.append(
                await client.post(
                    ...
                    content=b"structure",
                    headers={"x-crystalsketch-filename": "sample.cif"},
                )
            )
...
async def test_structure_atom_limit_has_stable_api_error(monkeypatch) -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={"x-crystalsketch-filename": "large.cif"},
        )
