...
import crystalsketch.server.app as app_module
import crystalsketch.server.prewarm as prewarm_module
import crystalsketch.structures.connectivity as connectivity_module
...
@pytest.mark.anyio
async def test_symmetry_endpoint_returns_analysis_without_replacing_scene() -> None:
    async with AsyncClient(
        transport=ASGITransport(app=create_app()), base_url="http://testserver"
    ) as client:
        response = await client.post(
            "/api/structure-symmetry",
            content=(FIXTURE_DIR / "Si.cif").read_bytes(),
            headers={"x-crystalsketch-filename": "Si.cif"},
        )
    assert response.status_code == 200
    assert response.json()["spaceGroupNumber"] == 227
    assert response.json()["crystalSystem"] == "cubic"
    assert "atoms" not in response.json()
    assert "bonds" not in response.json()


@pytest.mark.anyio
async def test_symmetry_endpoint_rejects_an_empty_upload() -> None:
    async with AsyncClient(
        transport=ASGITransport(app=create_app()), base_url="http://testserver"
    ) as client:
        response = await client.post(
            "/api/structure-symmetry", content=b"",
            headers={"x-crystalsketch-filename": "POSCAR"},
        )
    assert response.status_code == 400


@pytest.mark.anyio
async def test_structure_data_returns_full_cif_cell_without_display_images() -> None:
    from crystalsketch.structures.readers import read_structure
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/api/structure-data", content=(FIXTURE_DIR / "Si.cif").read_bytes(),
            headers={"x-crystalsketch-filename": "Si.cif"},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["cell"]["vectors"] == structure.lattice.matrix.tolist()
    assert len(data["sites"]) == len(structure)
    assert [site["fractionalPosition"] for site in data["sites"]] == structure.frac_coords.tolist()
    assert [site["siteId"] for site in data["sites"]] == [
        f"Si-{index}" for index in range(len(structure))
    ]


@pytest.mark.anyio
async def test_structure_data_rejects_partial_occupancy() -> None:
    from pymatgen.core import Lattice, Structure
    from pymatgen.io.cif import CifWriter

    structure = Structure(Lattice.cubic(5), [{"Si": 0.5}], [[0, 0, 0]])
    async with AsyncClient(
        transport=ASGITransport(app=create_app()), base_url="http://testserver"
    ) as client:
        response = await client.post(
            "/api/structure-data", content=str(CifWriter(structure)).encode(),
            headers={"x-crystalsketch-filename": "partial.cif"},
        )
    assert response.status_code == 422
    assert response.json()["detail"]["code"] == "disordered-structure"
...
def test_routes_import_defers_structure_preview_stack() -> None:
    result = subprocess.run(
        [
            ...
            (
                "import sys; "
                "import crystalsketch.server.routes; "
                "print('crystalsketch.structures.readers' in sys.modules); "
                "print('crystalsketch.structures.scene_builder' in sys.modules)"
            ),
        ],
        check=True,
        capture_output=True,
...
async def test_structure_preview_upload_endpoint_returns_scene() -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            content=payload,
            headers={"x-crystalsketch-filename": "SrTiO3.cif"},
        )
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={
                ...
                "x-crystalsketch-filename": "SrTiO3.cif",
            },
        )
...
async def test_structure_preview_upload_endpoint_applies_sparse_family_cutoffs() -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={
                "x-crystalsketch-filename": "SrTiO3.cif",
                "x-crystalsketch-bond-cutoff-overrides": '{"Sr|O":{"min":0,"max":2}}',
            },
        )
...
async def test_structure_preview_upload_endpoint_rejects_invalid_family_cutoffs() -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={
                "x-crystalsketch-filename": "SrTiO3.cif",
                "x-crystalsketch-bond-cutoff-overrides": '{"Sr|O":{"min":2,"max":2}}',
            },
        )
...
@pytest.mark.anyio
async def test_explicit_connectivity_endpoint_fails_instead_of_returning_ready_without_bonds(
    monkeypatch,
) -> None:
    payload = (FIXTURE_DIR / "SrTiO3.cif").read_bytes()
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/api/structure-preview?includeConnectivity=true",
            content=payload,
            headers={"x-crystalsketch-filename": "SrTiO3.cif"},
        )
    ...
    assert response.json()["detail"] == {
        "code": "bond-recalculation-failed",
        "message": (
            "Connectivity calculation failed: Bond analysis with CrystalNN "
            "failed: neighbor graph unavailable"
        ),
    }
...
async def test_structure_preview_upload_endpoint_returns_parse_error() -> None:
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={"x-crystalsketch-filename": "bad.cif"},
        )
...
async def test_structure_preview_upload_endpoint_rejects_oversized_payload() -> None:
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            headers={"x-crystalsketch-filename": "movie.mp4"},
        )
...
async def test_static_index_is_served_from_explicit_static_root(tmp_path) -> None:
    ...
    (tmp_path / "index.html").write_text("<!doctype html><title>CrystalSketch</title>")
    (tmp_path / "favicon.svg").write_text("<svg><title>CrystalSketch logo</title></svg>")
    ...
    async with AsyncClient(
        ...
        assert response.status_code == 200
        assert "CrystalSketch" in response.text
        assert fallback_response.status_code == 200
        assert "CrystalSketch" in fallback_response.text
        assert favicon_response.status_code == 200
        assert "CrystalSketch logo" in favicon_response.text
...
async def test_missing_static_root_returns_actionable_page(tmp_path) -> None:
    async with AsyncClient(
        ...
        assert "Crystal --verbose" in response.text
