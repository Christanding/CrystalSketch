...
from crystalsketch.cli import _choose_port, _wait_for_server
...
def test_cli_import_defers_server_app() -> None:
    result = subprocess.run(
        [
            ...
            (
                "import sys; "
                "import crystalsketch.cli; "
                "print('crystalsketch.server.app' in sys.modules)"
            ),
        ],
        check=True,
        capture_output=True,
...
def test_run_gui_handles_keyboard_interrupt_without_traceback(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert "CrystalSketch stopped" not in result.output
...
def test_root_version_option_prints_current_version(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert result.output == f"CrystalSketch {__version__}\n"
...
def test_help_accepts_short_option() -> None:
    ...
    assert "Start the CrystalSketch local GUI." in root_result.output
    assert "gui" not in root_result.output
    assert "Start the local CrystalSketch GUI server." in gui_result.output
...
def test_open_browser_when_ready_reports_unavailable_server(monkeypatch, capsys) -> None:
    ...
    assert "Crystal --verbose" in error_output
